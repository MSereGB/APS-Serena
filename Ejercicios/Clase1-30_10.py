...
# Llamada a firwin2: no pasar nfreqs (el valor previo producía nfreqs < ntaps)
# si necesitas control de nfreqs, asegúrate de que nfreqs > cant_coef
fir_win_rect = signal.firwin2(numtaps=cant_coef, freq=frecuencias, gain=deseado, window='boxcar', fs=fs)
...
# Recalcular retardo después de la convolución (la longitud del filtro cambió)
retardo = (len(fir_win_rect) - 1)//2
...
for ii in regs_interes:
    ...
    # Indexado seguro para evitar IndexError si zoom_region+retardo supera la longitud
    idx = (zoom_region + retardo).astype(int)
    idx = np.minimum(idx, len(ecg_filt_win) - 1)
    plt.plot(zoom_region, ecg_filt_win[idx], label='FIR Window')
